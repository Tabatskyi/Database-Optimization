...
def get_conn(dbname: str = TARGET_DB):
    ...
    config["connect_timeout"] = int(os.getenv("POSTGRES_CONNECT_TIMEOUT", "5"))
    return psycopg2.connect(**config)


def wait_for_postgres(max_attempts: int = 30, sleep_seconds: float = 1.0) -> None:
    """Wait until the compose-managed PostgreSQL container is ready."""
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt == max_attempts:
                raise RuntimeError(
                    f"Unable to connect to PostgreSQL after {max_attempts} attempts: {exc}"
                ) from exc
